...
input = {
    ...
    "res": "2,4,1,3,7,5,1,5,0,3,4,1,5,5,3,0",
}
...
milestone = 0
partials = {}
while result != res and itr < 10:
    itr += 1
    # if len(result) < len(res):
    #     count *= 8
    # else:
    print(
        result[subcount:], res[subcount:], result[subcount:] == res[subcount:], subcount
    )
    if result[subcount:] == res[subcount:]:
        key = ",".join(res[subcount:])
        if key in partials:
            partials[key].append(count)
        else:
            partials[key] = [count]
    count += pow(8, subcount)
    if len(result) > len(res) or subcount == 0:
        print("FAIL")
        break
    a = count
    print(a, result)
